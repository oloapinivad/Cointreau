...
for (country in countries) {
  ...
  } else if (country == "France" | country == "United Kingdom" | country == "Netherlands") {
  ...
  out_country[[country]] <- predict.covid(world_plot_calendar, death, total_length = end, name = country, verbose = T)
...
ggsave(
  filename = file.path(FIGDIR_WOR, paste0("status_world_", dates[length(dates)], ".pdf")), plot = ml,
  width = 10, height = 10
)
...
ggsave(file = file.path(FIGDIR_WOR, paste0("world_COVID_prediction_day_", dates[length(dates)], ".pdf")), 
...
for (last_day in 20:length(full_death)) {
  ...
  out_italy <- predict.covid(plot_calendar, death, total_length = end, name = "Italy", verbose = T)
...
theplot[[2]] <-  italy_daily[[2]] <- ggplot(df) +
  ...
  ggsave(file = file.path(FIGDIR_ITA, paste0("Italy_COVID_prediction_day_", full_calendar[last_day], ".pdf")),
    ml, height = 10, width = 12)
...
for (region in regions) {
  ...
  out_region[[region]] <- predict.covid(regions_plot_calendar, death, total_length = end, name = region, verbose = T)
...
ggsave(file = file.path(FIGDIR_REG, paste0("italian_regions_COVID_prediction_day_", dates[length(dates)], ".pdf")),
ml, height = 10, width = 12)


 ml <- arrangeGrob(grobs = theplot, ncol = 2, nrow = 4, plot = F)
  ggsave(file = file.path(FIGDIR, "covid_status_today.svg"), plot = ml, height = 20, width = 26, system_fonts = list(sans = "Arial"))
